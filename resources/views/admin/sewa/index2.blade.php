        ...
        <table class=" table w-full">
                ...
                <dialog id="my_modal_5" class="modal modal-bottom sm:modal-middle">
                    <div class="modal-box">
                        ...
                        <div class="modal-action">
                            <form method="POST" action="{{ url('laporan-b') }}">
                                @csrf
            ...
            @foreach ($data as $item)
                <tr class="hover">
                    ...
                    <td class="flex gap-4">
                        ...
                        <dialog id="konfirmasi_pembayaran{{ $item->id }}" class="modal modal-konfirmasi">
                            <div class="modal-box w-11/12 max-w-3xl">
                                ...
                                <div class="modal-action">
                                    <form method="post" action="{{ url('konfirmasi-pembayaran2/' . $item->id) }}"
                                        method="POST">
                                        @csrf
                        ...
                        <dialog id="changeFile{{ $item->id }}" class="modal modal-bottom sm:modal-middle modal-file">
                            <div class="modal-box">
                                ...
                                <p>Tekan ESC untuk close</p>
                                <div class="modal-action">
                                    <form method="post" action="{{ url('change-bukti2/' . $item->id) }}"
                        ...
                        <dialog id="my_modal_4{{ $item->id }}" class="modal">
                            <div class="modal-box w-11/12 max-w-3xl">
                                ...
                                <div class="modal-action">
                                    <form method="post" action="{{ url('delete-data-jadwalb/' . $item->id) }}"
                                        method="POST">
                                        @csrf
                                        <!-- if there is a button, it will close the modal -->
                    ...
                    <!-- Modal -->
                    <divcloseclose
                        class="modal-image-container fixed z-10 inset-0 bg-black bg-opacity-75 flex justify-center items-center hidden">
                        <span class="close-modal absolute top-4 right-4 text-white text-3xl cursor-pointer">&times;</span>
                        <img src="{{ asset('bukti_pembayaran/' . $item->bukti_pembayaran) }}"
                            class="modal-image max-w-full max-h-full" alt="Full Image" />
                    </divcloseclose>
